// packages/net/src/safe-fetch.ts
//
// The SSRF gate. Authority: Eng §11.3, blocklist enumerated in Feature Spec
// §6.3. Every outbound request made on behalf of a user-supplied URL goes
// through here - there is no second path, and adding one is a review-blocking
// defect.
//
// The threat is plain: our services run inside Railway's network with a
// database, a Redis instance and a cloud metadata endpoint reachable from them,
// and users hand us arbitrary URLs to fetch. Without this gate, "monitor this
// URL" is a request-forgery primitive pointed at our own infrastructure.
//
// TWO CALLERS, ONE GATE. The worker polls these URLs on a schedule; the api
// fetches one on demand so the custom widget's config form can show the user
// what fields the response actually has. The api's path is the more exposed of
// the two - it is reachable by any signed-in user, at a time of their choosing,
// with a URL they just typed - which is exactly why it uses this and not its
// own fetch.
//
